...
from packet_cont_learning import PacketYaTC, build_supcon_dl
...
#20
    ...
    parser.add_argument('--is_supcon', action='store_true', default=False, help='Use supervised contrastive learning')
    parser.add_argument('--supcon_temp', default=0.07, type=float, help='temperature for supervised contrastive learning')

    return parser

def train_one_epoch_supcon(model: torch.nn.Module, criterion: torch.nn.Module,
...
def main(args):
    ...
    # supcon data loader
    if args.is_supcon:
        data_loader_train = build_supcon_dl(args.data_path, args.batch_size, is_train=True)
        data_loader_val = build_supcon_dl(args.data_path, args.batch_size, is_train=False)
    ...
    if args.is_supcon:
        criterion = SupConLoss(temperature=args.supcon_temp)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.is_supcon:
            train_stats = train_one_epoch_supcon(
